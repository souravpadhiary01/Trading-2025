...
public partial class teammonth : System.Web.UI.Page
{
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static string InsertPaymodal(InvestorPaymodal data)
    {
        if (data == null || string.IsNullOrEmpty(data.Uploadpicfile) || string.IsNullOrEmpty(data.Uploadpathfile))
        {
            return "File data or file name is missing.";
        }
        ...
        {
            // Upload file to FTP
            string filePath = UploadFileToFtp2(data.Uploadpicfile, data.Uploadpathfile);
            if (filePath == null)
            {
                return "File upload failed.";
            }
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                string query = @"
                INSERT INTO teamtransction 
                (membername, memberid, transctionamount, transctiontype, transctionby, phato, remark, ClientPayments, ClientFund) 
                VALUES 
                (@membername, @memberid, @transctionamount, @transctiontype, @transctionby, @phato, @remark,@ClientPayments, @ClientFund)";
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@transctionby", data.transctionby ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@remark", data.remark ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@ClientPayments", data.payClientPayments ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@ClientFund", data.payClientFund ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@phato", filePath); // File path from FTP upload
    ...
    private static string UploadFileToFtp2(string base64Data, string fileName)
    {
        if (string.IsNullOrEmpty(base64Data) || string.IsNullOrEmpty(fileName))
        {
            Debug.WriteLine("Base64 data or file name is missing.");
    ...
    public class InvestorPaymodal
    {
        ...
        public string transctionby { get; set; } // Matches "transctionby" column
        public string remark { get; set; } // Matches "remark" column
        public string payClientPayments { get; set; }
        public string payClientFund { get; set; }
        public string Uploadpicfile { get; set; } // Base64 file data
        public string Uploadpathfile { get; set; } // File name (to be used for "filePath" column)
    }
